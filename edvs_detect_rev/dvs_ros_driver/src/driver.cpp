...
namespace dvs_ros_driver
{
	...
	DvsRosDriver::DvsRosDriver(ros::NodeHandle &nh, ros::NodeHandle nh_private) : nh_(nh), parameter_update_required_(false)
	{
		// load parameters
		...
		while (!device_is_running)
		{
			...
			dvs128_handle = caerDeviceOpenSerial(1, CAER_DEVICE_EDVS, serialPortName.c_str(), CAER_HOST_CONFIG_SERIAL_BAUD_RATE_12M);
			// dvs128_handle = caerDeviceOpen(1, CAER_DEVICE_DVS128, 0, 0, serial_number_restrict);
